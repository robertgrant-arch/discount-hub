...
const CACHE_KEY = "dh_zip_cache";
const CACHE_TTL = 7 * 24 * 60 * 60 * 1000; // 7 days

function getCache(): Record<string, { data: ZipInfo; ts: number }> {
  try {
...
function setCache(zip: string, info: ZipInfo) {
  try {
    const cache = getCache();
    cache[zip] = { data: info, ts: Date.now() };
...
function getCached(zip: string): ZipInfo | null {
  const cache = getCache();
...
// US state abbreviation → full name map (for API response normalization)
...
const STATE_NAME_TO_ABBR: Record<string, string> = Object.fromEntries(
  Object.entries(STATE_ABBR).map(([abbr, name]) => [name.toUpperCase(), abbr])
);
...
export async function lookupZip(zip: string): Promise<ZipInfo | null> {
  ...
  try {
    ...
    const stateName = place["state"] as string;
    ...
    const info: ZipInfo = {
      ...
      city,
      state: stateAbbr,
      stateName: STATE_ABBR[stateAbbr] || stateName,
      country: "US",
    };

    setCache(cleaned, info);
    return info;
  } catch {
    return null;
  }
...
// Synchronous fallback using first 3 digits of zip for rough state estimation
...
const ZIP_PREFIX_STATE: Record<string, string> = {
  ...
  "875": "NM", "876": "NM", "877": "NM", "878": "NM", "879": "NM",
...
// Synchronous fallback: estimate state from zip prefix (first 3 digits)
export function getStateFromZipSync(zip: string): string | null {
  const cleaned = zip.trim().replace(/\D/g, "").slice(0, 5);
...
export function getStateNameFromAbbr(abbr: string): string {
  return STATE_ABBR[abbr] || abbr;
}
